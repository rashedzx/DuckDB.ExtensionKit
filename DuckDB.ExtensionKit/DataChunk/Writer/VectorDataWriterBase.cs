...
internal unsafe class VectorDataWriterBase(IntPtr vector, void* vectorData, DuckDBType columnType) : IDisposable, IDuckDBDataWriter
{
    ...
    public void WriteNull(ulong rowIndex)
    {
        if (validity == default)
        {
            NativeMethods.NativeMethods.Vectors.DuckDBVectorEnsureValidityWritable(Vector);
            validity = NativeMethods.NativeMethods.Vectors.DuckDBVectorGetValidity(Vector);
        }

        NativeMethods.NativeMethods.ValidityMask.DuckDBValiditySetRowValidity(validity, rowIndex, false);
    }
    ...
    {
        validity = default;
        vectorData = NativeMethods.NativeMethods.Vectors.DuckDBVectorGetData(Vector);
    }
