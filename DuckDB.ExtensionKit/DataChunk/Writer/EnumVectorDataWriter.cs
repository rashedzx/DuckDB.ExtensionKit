...
{
    private readonly DuckDBType enumType = NativeMethods.NativeMethods.LogicalType.DuckDBEnumInternalType(logicalType);

    private readonly uint enumDictionarySize = NativeMethods.NativeMethods.LogicalType.DuckDBEnumDictionarySize(logicalType);
    ...
    internal override bool AppendString(string value, ulong rowIndex)
    {
        if (enumValues.Count == 0)
        {
            ...
            {
                var enumValueName = PointerExtensions.ToManagedString(NativeMethods.NativeMethods.LogicalType.DuckDBEnumDictionaryValue(logicalType, index));
